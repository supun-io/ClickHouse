    ...
    std::optional<CurrentThread::QueryScope> & query_scope)
{
    ...
    size_t buffer_size_total = std::max(
        std::max(params.getParsed<size_t>("buffer_size", 0), context->getSettingsRef().http_response_buffer_size),
    ...
    bool buffer_until_eof = params.getParsed<bool>("wait_end_of_query", false) || context->getSettingsRef().http_wait_end_of_query;
