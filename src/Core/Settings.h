...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, http_response_buffer_size, false, "The number of bytes to buffer in the server memory before sending a HTTP responce to the client or flushing to disk (when http_wait_end_of_query is enabled).", 0) \
    \
